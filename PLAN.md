...
**The application has now held the port, and V2–V3 are green.** Handshake reached `ready` with the correct identity fields, twice, from independent connections. `TEST 1` and `TEST 4` round-tripped through the live app with zero sequence gaps and zero duplicates, `SILENT`/plain `ACK` split correctly on the loaded ruleset's inert button, and the `F2` pending-choice assign/clear cycle confirmed on the wire (§5, V2–V3). **Still outstanding: V4–V6**, which need physical acts — unplug, sleep, kill-the-browser — that are the operator's hands, not a script's.

...
**Status as of 2026-08-13.** `SCOPE.md` and `SYSTEM_FUNC_SPEC.md` are the authority on direction; `PROTOCOL.md` **v3.0** is the revision that answers to them, and it is a breaking change against the v2.0 prototype. The scoreboard application is at v3.0 (M1, complete); the dongle firmware is at v3.0, flashed and answering on hardware. The radio layer — `RADIO_PROTOCOL.md` **v1.0** — is implemented and, for the first time, **confirmed on hardware**: a DK remote (RED) built per `remote/BUILD_SPEC.md`, flashed, and connecting to the dongle over BLE, encrypted and reporting `CONNECTED` to the scoreboard app — W1's positive case, §1. GREEN and W1's four negative cases (A12–A14, A19) are still open, and A13's own check turned out to be unimplemented rather than merely untested (§8).
...
| **M2** | **The firmware programme** — dongle wire v3.0, the radio, and a DK remote, to an end-to-end demonstration | ▶ in progress — §3.1. **Stage 1 (the no-radio baseline) is closed**; V0–V6 all run, with V4 row 7, V5.1's remote-render half, V6.4 and V6.5 carried forward as deferred rather than blocking (§5, §9.2). **Stage 2 (provisioning) is closed 2026-08-12** — **A19 confirmed on hardware**, both the unprovisioned fallback and a written record, dongle side (§5, §9.2). **Stages 3 and 4 are code-complete 2026-08-12; W0 is green.** **W1's positive case is confirmed on hardware, 2026-08-13** — one DK (RED) connects, encrypts, discovers, subscribes and reports `CONNECTED` with a real RSSI and telemetry to the scoreboard, with `LED_LINK` (DK LED 4) agreeing from the remote's own side. Getting there surfaced a real firmware bug, not a bench mistake: `radio_init()` fired both remotes' `bt_conn_le_create()` unconditionally at boot, but Zephyr allows only one outstanding initiating procedure system-wide, and this project's own "never stops trying" search could — with only one remote actually present — permanently starve the other's turn at that one slot. Fixed with a round-robin scheduler, §9.2/§9.3 2026-08-13. **W1's negative cases (A12–A14, A19) are still open, and A13 cannot pass as currently written** — `handle_identity_read()`'s `set_serial` comparison is a known, deliberately-deferred no-op (§8). W2–W5 remain unconfirmed; GREEN isn't needed for W1. **Next: implement the A13 check, then work the four negative cases** |
...
| Dongle radio | **Code-complete 2026-08-12, W1 positive case confirmed on hardware 2026-08-13.** `src/radio_ble.c` implements the full connection lifecycle (`dongle/BUILD_SPEC.md` §7.1) behind `CONFIG_DONGLE_RADIO=y`; builds clean, no-radio baseline (`radio_null.c`) unaffected. `CONFIG_DONGLE_FAKE_LINK` remains **deleted** (§4.11) — nothing fabricates a connection here either. **The single-producer workqueue invariant now holds for real**: every `bt_conn_cb`/GATT client callback marshals onto `radio_workq` via a `k_msgq` + one static `k_work`, mirroring `usb_link.c`'s RX pattern, rather than touching `remote_state` from the BT host thread (§9.3, 2026-08-12). **The two-remote connection scheduler was rewritten 2026-08-13**: Zephyr allows only one outstanding `bt_conn_le_create()` system-wide, so the original boot-time "fire both, let each search forever" logic could let one remote's unbounded search for an absent peer permanently starve the other, even a present and correctly-advertising one. `connect_owner`/`connect_yield` now share that one slot in bounded round-robin turns only when both remotes actually want it at once; an uncontested search (the realistic mid-match case — one remote drops, the other stays connected) still runs unbounded exactly as `RADIO_PROTOCOL.md` §9.5 always intended (§9.2, §9.3) |
| Remote firmware | **Code-complete 2026-08-12, confirmed connecting on hardware 2026-08-13 (RED).** Full DK remote per `remote/BUILD_SPEC.md`: GATT server (`link.c`), gesture classifier (`buttons.c`), PWM haptic proxy preserving the BEAT:TAP ratio (`haptic.c`), `LED_LINK` conjunction (`indicators.c`), synthetic telemetry (§8.3, confirmed on the wire as the fixed `42` `SYNTHETIC_BATTERY_PCT`). Builds clean against `nrf52840dk/nrf52840`. Custom hardware still not designed — this is the DK prototyping platform the milestone table always specified. **Same workqueue fix as the dongle** — `link.c`'s `bt_conn_cb`/GATT server callbacks marshal onto `link_workq` instead of touching module state from the BT host thread; `identity_read_cb` is the one deliberate exception (§9.3). **GREEN not yet built/flashed** — not needed for W1 |
| Provisioning | **Redesigned 2026-08-12 (§4.13 reverses §4.10), confirmed on hardware both sides 2026-08-13.** Identity is baked into the firmware image at build time: `dongle/tools/provision.py` generates a `provisioning_data.h` per unit, `CMakeLists.txt` refuses to configure without one, and `provisioning_validate()` (`common/provisioning.h/.c`, 9 host checks) is a boot-time structural sanity check, not a flash-record parser. **A dongle and a DK (RED), flashed from the same generated set via `tools/build_set.ps1`, now associate and stay connected** — the positive case, on real hardware, under the new mechanism, both ends. Tooling: `tools/build_set.ps1` collapses generate-then-build-times-three-then-package-the-DFU-zip into one command; `tools/ncsenv.ps1` is the PowerShell equivalent of `dongle/tools/ncsenv.sh`. Neither sets `NRFUTIL_HOME` — `CLAUDE.md` §6 has the trap that cost real time finding out why. |
...
**Consequence for stage 2/3's status:** A19 and A12/A13 remain testable exactly as before — generate a deliberately-wrong `provisioning_data.h` (bad role, mismatched set) and build against it, in place of generating a mismatched hex. W0 (`dongle/tests/rframe`) is unaffected; `dongle/tests/provisioning`'s 54 CRC/byte-layout checks are replaced by 9 checks against `provisioning_validate()` directly (§9.2). Both firmwares rebuild smaller with the flash-reading machinery gone — dongle radio=y 181,668 B (was 185,212 B), no-radio baseline 54,936 B (was 56,200 B), remote 148,468 B (was 155,628 B) — and `CONFIG_FLASH`/`CONFIG_FLASH_MAP` are dropped from both `prj.conf`s, nothing else on either board reads a flash partition.
...
| **Provisioning A19 unconfirmed on hardware under the new mechanism** | The stage-2 flash-partition design's hardware confirmation is in doubt (§4.13: the DFU write likely never reached `storage_partition`). Its replacement — identity baked in at build time, `provisioning_validate()` as a boot-time sentinel — is host-verified (9 checks) and confirmed on hardware for the *correctly*-provisioned case (§9.2, 2026-08-13). **A19's actual refuse-to-operate behavior (the all-zero-key sentinel) has not been run on a physical unit** — the case tested so far is a valid record, not an invalid one. §3.1 stage 2, §4.13 |
| **A13 (`RR_IDENTITY` set-serial mismatch) is not implemented** | `dongle/src/radio_ble.c`'s `handle_identity_read()` has a comment recording the comparison as deliberately deferred — the dongle's own `set_serial` was never available at that call site to compare against. A remote provisioned for a different set currently associates and connects normally instead of producing `ERR SET_MISMATCH`, so this negative case cannot be run until the comparison is added. Found 2026-08-13 while planning the negative-case bench walkthrough, not by running it and failing. `RADIO_PROTOCOL.md` §14, W1 |
...
- [ ] W1 pass *including* its negative cases (M2 stage 3) — **positive case confirmed on hardware 2026-08-13** (§9.2); A12/A13/A14/A19 outstanding, and A13's own check is unimplemented (§8)

...
| Radio conformance tests | **Harness written, 2026-08-12** — `dongle/tests/rframe/`, 20 checks covering A1–A7 and A11 (`common/rframe.c`'s codec-level cases; A20 is structural — the codec offers no way to construct a silent tap). **Not yet run**: the host MinGW `as.exe` returns `Permission denied` in the environment that wrote it, an OS/AV-level block unrelated to the code — `gcc.exe` itself runs fine, and a fresh copy of `as.exe` in a clean directory fails identically. Independently cross-checked instead, in a throwaway Python reimplementation of the codec and CTR logic (13/13, including one real fixture bug the cross-check caught — §9.3). W0 needs a `make check` run on a machine where the toolchain isn't blocked. |
...
| Stage | Delivers | Exit |
|---|---|---|
| **0** | Host toolchain; both host suites rewritten to v3.0 **before** the parser is touched | ◐ **2026-08-11** — toolchain done, protocol suite done and green (§2.6). **The `rframe` suite is written at stage 3** (below), where the codec it tests is designed |
...
| **3** | `rframe` codec, then BLE at 7.5 ms | ◐ **Code-complete 2026-08-12.** `common/rframe.c/.h` (both directions, CTR arithmetic) and `dongle/tests/rframe/` (20 checks covering A1–A7, A11) written; `dongle/src/radio_ble.c` implements the full connection lifecycle (`dongle/BUILD_SPEC.md` §7.1) and builds clean under `-DCONFIG_DONGLE_RADIO=y`, no-radio baseline unaffected. **W0 not yet formally run** — the host `as.exe` is blocked in the environment that wrote this (`Permission denied`, looks like an AV/OS block, not a code defect); the codec was independently cross-checked in a throwaway Python reimplementation instead (13/13, including the CTR fixture bug that check caught and fixed — see §9.3). **W1 unconfirmed** — needs two provisioned units and a radio scan/join, i.e. hardware. §9.2 |

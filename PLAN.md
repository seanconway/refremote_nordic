...
**Status as of 2026-08-13.** `SCOPE.md` and `SYSTEM_FUNC_SPEC.md` are the authority on direction; `PROTOCOL.md` **v3.0** and `RADIO_PROTOCOL.md` **v1.0** answer to them. The scoreboard application is at v3.0 (M1, complete). **M2 and M4's fast path are both done**: the dongle firmware is at v3.0 with the radio confirmed on hardware end to end (W0–W3), the M2 demonstration has run (press → score → tap, on that DK only), and the 2:1 link is proven — both connections hold simultaneously and route correctly, confirmed under a full colour swap between the DK and the PCA10059 (S9). **The development bottleneck is PCB fabrication lead time, not bench validation** (§4.16): the near-term line is now S13–S19 (M5, the full-feature DK remote) → M6's PCB design and order. W4/W5's edge-case matrix, the full W6 characterisation, W7 range/density, and the rest of M4's deeper validation all still run — during the PCB fab wait, on firmware that has stopped changing shape (S20, S10–S12). **The next step is S13.**

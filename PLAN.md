...
**The dongle was flashed to 0.2.0 on 2026-08-11 and the wire layer answers correctly on hardware.** `INFO` returns `HELLO 3.0 0.2.0 RR-0000 0`; `TEST 1` emits seven events and `TEST 4` emits thirty-two; supervision fires at 2512 ms measured; the 120 ms acknowledgement budget shows all three of its bands. **The haptic downlink renders and is correctly routed per remote**, proven by the `RED`-dark/`GREEN`-lit asymmetry that the board's single fitted LED makes visible ([`dongle/BOARD.md`](dongle/BOARD.md) §2.1). **The emulator wire-log diff is clean** — every difference between firmware and `dongleModel.js` is accounted for, and one of them was predicted in advance (§2.7). **Still outstanding**: the `STATE` render, two commands away; and every rung needing the application (V2, V4, V5, V6).
...
### V1 — Manual terminal, no browser — ⚠️ **WIRE GREEN 2026-08-11, HAPTIC RENDER AND ROUTING GREEN, `STATE` UNOBSERVED**

Every wire response below was confirmed against firmware 0.2.0 on 2026-08-11, driven from a scripted terminal rather than typed, so the timings are recorded rather than eyeballed — §2.7.

**The haptic half is closed.** `HAP GREEN LONG` blinks the lamp and `HAP RED LONG` does not, which establishes both that `indicator.c` renders and that it honours the target — a firmware driving both channels would have blinked on both ([`dongle/BOARD.md`](dongle/BOARD.md) §2.1).

**One thing keeps this rung open, and one thing is unclosable here.** `STATE` has only been seen to be *accepted*, which is byte-identical on the wire to a dead indicator — but it **is** observable, on the `GREEN` channel, and closing it is two commands (below). `CFG` is **not** observable on this board at all: `indicator.c` drives GPIO, not PWM, so a brightness or amplitude scale has nothing to act on. Record `CFG` as out of this rung's reach rather than pending; it first becomes observable at stage 4.

```
TEST 3                                    suspend supervision
STATE GREEN SOLID 00FF00 OFF 000000       → lamp takes a steady level and HOLDS it
STATE GREEN OFF 000000 OFF 000000         → lamp goes out
TEST 0                                    MANDATORY
```

Addressed to `GREEN` deliberately — `STATE RED` drives the unfitted pin and would show nothing whether it worked or not.
...
| 2026-08-11 | 0.2.0 | 3.0 | **V1** | **partial** | **Wire green.** `HELLO 3.0 0.2.0 RR-0000 0`; `PONG`; `ECHO` verbatim with runs of spaces intact; `ERR APP_TIMEOUT` at **2512 ms**; malformed lines rejected with a reason. **Haptic render and per-remote routing green** — see the board row below. **Not closed:** the `STATE` render, which is two commands away; `CFG` and `HAP RED` are unobservable on this board and are reassigned to stage 4 — §2.7 |

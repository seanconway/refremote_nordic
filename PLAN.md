...
| **M2** | **The firmware programme** — dongle wire v3.0, the radio, and a DK remote, to an end-to-end demonstration | ▶ in progress — §3.1. **Stage 1 (the no-radio baseline) is closed**; V0–V6 all run, with V4 row 7, V5.1's remote-render half, V6.4 and V6.5 carried forward as deferred rather than blocking (§5, §9.2). **Stage 2 (provisioning) is closed 2026-08-12** — **A19 confirmed on hardware**, both the unprovisioned fallback and a written record, dongle side (§5, §9.2). **Next: stage 3 (`rframe` codec, then BLE at 7.5 ms)** |

...
| **M2** | **The firmware programme** — dongle wire v3.0, the radio, and a DK remote, to an end-to-end demonstration | ▶ in progress — §3.1. Stages 0–1 code-complete and **flashed**; V0–V5 green (V4 minus one UI-unreachable row, V5.1's remote-render half deferred to stage 4); **V6 remaining**, needs physical acts |
...
| Validation | **V0–V5 all green**, 2026-08-11 — V0/V1 from a scripted terminal (§2.7), V2–V5 from the live application holding the port (§5, §9.2). The v2.0 passes of V1–V3 are **void** and have been superseded rather than carried. **V4's one open row — burst suppression under `ADD_POINT`s — cannot be driven from the operator UI at all**, since an on-screen click produces no wire traffic to acknowledge; it needs dongle-originated `EVT`s and is otherwise covered by a unit test. **V5's one open row — "both remotes render link-lost" on `ERR APP_TIMEOUT`** — has no remote to render on yet; `radio_null.c` says so directly and the check moves to stage 4 as W5/A15. **V6 needs physical acts** — unplug, replug, ten reconnects, a dongle swap — that are the operator's hands, not a script's; V6.1/V6.3/V6.4 are the exception, reachable from the UI alone. §5 carries the rungs as per-stage exit criteria. |

**The two ends have been connected, in both directions.** The dongle runs 0.2.0 and speaks v3.0, the application has held the port multiple times independently, and V2–V5 are closed. **V6 — reconnect lifecycle — is the last rung of the no-radio baseline**, and it needs the operator's hands: unplug, replug, ten reconnects in a row, a dongle swap if a second unit is available.
...
| **1** | Wire v3.0, the `radio.h` seam, `radio_null` | ◐ **V0–V5 green, 2026-08-11.** Emulator wire-log diff clean (§2.7). Outstanding: **V6** only |

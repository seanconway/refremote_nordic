...
| Provisioning | **Redesigned 2026-08-12 — §4.13 reverses §4.10.** Attempting to write the stage-2 mechanism's provisioning hex to a real dongle for W1 surfaced that this board's Serial DFU bootloader activates writes into the application slot regardless of the address the hex file claims, so the record was very likely never actually reaching `storage_partition` — the stage-2 "confirmed on hardware" positive case (§9.2, 2026-08-12) is now in doubt for that reason; the unprovisioned/A19 negative case, which needs only erased flash, is not. Identity is now baked into the firmware image at build time instead: `dongle/tools/provision.py` generates a `provisioning_data.h` per unit, `CMakeLists.txt` refuses to configure without one, and `provisioning_validate()` (`common/provisioning.h/.c`, 9 host checks) is a boot-time structural sanity check, not a flash-record parser. Both firmwares build clean against a generated set. **Not yet confirmed on hardware under the new mechanism** — that's the immediate next step, and it uses the one DFU path already proven to work (writing the application slot). |
...
| **2** | Provisioning record, reader, refusal path, bench tool | ◐ **Redesigned 2026-08-12 — §4.13.** `common/provisioning.c` now holds `provisioning_validate()` (9 host checks) against a compile-time `PROV_RECORD`, not a flash-record parser; `dongle/src/provisioning_flash.c` is deleted. Reason: the stage-2 DFU write of a provisioning hex to `storage_partition` doesn't reach it on this bootloader (§4.13), which puts the original "A19 confirmed on hardware" positive case in doubt — the negative/unprovisioned case is unaffected, since it needs only erased flash. **A19 on hardware under the new mechanism is the open rung**, using the DFU path already proven to work (the application slot) |

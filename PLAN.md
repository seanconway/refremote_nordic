...
| **M2** | **The firmware programme** — dongle wire v3.0, the radio, and a DK remote, to an end-to-end demonstration | ▶ in progress — §3.1. **Stage 1 (the no-radio baseline) is closed**; V0–V6 all run, with V4 row 7, V5.1's remote-render half, V6.4 and V6.5 carried forward as deferred rather than blocking (§5, §9.2). **Stage 2 (provisioning) is code-complete 2026-08-12** — host suite green, `provision.py` output verified byte-for-byte through the parser, no-radio build clean; **not yet flashed to hardware**. **Next: A19 on hardware, then stage 3** |
...
| Provisioning | **Code-complete 2026-08-12, not yet on hardware.** `common/provisioning.h/.c` (Zephyr-free record, CRC32, validation — 54 host checks) and `dongle/src/provisioning_flash.c` (the `storage_partition` reader, wired into `engine_start()`) exist; `HELLO`'s `<set>` field and `ERR NO_PROVISIONING` behave per `BUILD_SPEC.md` §9. `dongle/tools/provision.py` generates a set's three records and its output round-trips through the parser unchanged. **Not yet run:** writing a record to a real board and observing it — dongle/BUILD_SPEC.md §9. It remains a prerequisite of the radio, not of manufacture — §4.10. |
...
**The two ends have been connected, in both directions, and the no-radio baseline is closed.** The dongle runs 0.2.0 and speaks v3.0, the application has held the port repeatedly and reconnected through every scenario that doesn't need scripting or a second unit. **Stage 2 — provisioning — is code-complete off hardware**: the record parser, the flash reader, and the bench tool are all written and cross-checked against each other on the host, but the physical dongle still runs 0.2.0 without a provisioning record and nobody has watched `ERR NO_PROVISIONING` fire on real hardware yet. That flash-and-observe step, then stage 3, is next per §3.1's table.
...
| **2** | Provisioning record, reader, refusal path, bench tool | ◐ **Code-complete 2026-08-12** — `common/provisioning.c` (54 host checks), `dongle/src/provisioning_flash.c` wired into `engine_start()`, `provision.py` verified byte-for-byte through the parser, no-radio build clean (`dongle/BUILD_SPEC.md` §9). **Outstanding: A19 on hardware** — a real board has not yet been flashed with either a valid or an absent record. The remote half of "on both boards" needs stage 4 |
...
| ~~No provisioning record, reader, or tool~~ | **Code-complete 2026-08-12** — `common/provisioning.c`, `dongle/src/provisioning_flash.c`, `dongle/tools/provision.py`, all cross-verified on the host. **Residual gap: unverified on hardware** — A19 has not been observed on a real board, and the remote-side half needs stage 4 firmware — §3.1 stage 2, §4.10 |

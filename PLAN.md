...
| **Remote fails to reconnect after a dongle power-cycle** (reported 2026-08-14) | Static read of `radio_ble.c`'s connect/retry state machine and `link.c`'s advertising restart, both independently (agent + direct read), found no logical defect — cold boot and reboot are symmetric in the code, and `RADIO_PROTOCOL.md` §9.5's one real subtlety (the RED/GREEN shared-slot round-robin) is documented and shouldn't cost more than a few seconds. `PLAN.md`/`HISTORY.md` confirm this exact scenario (dongle rebooted, remote left powered) was never bench-tested — only adjacent cases (remote power-cycle, app USB reconnect) are | A bench session with the dongle's LOG output and the remote's RTT console both visible during a repro, timing actual recovery | Next bench session touching the radio; not blocking S15+ |

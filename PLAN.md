...
| **M5** | Full-feature remote firmware on the DK — GPIO buttons, RGB indicators, an LRA (the ERM retained as the fallback candidate), nPM1300 | queue §2.3, S13–S19 |
...
| 4 | **M5** — S13–S19 | The remote's real peripherals — seven buttons, RGB, the haptic motor (LRA — the already-built ERM path stays on the bench as the fallback candidate), a PMIC. A failure is hardware or drivers, not protocol. Also produces M6's design-target estimates for R3/R4/R6 (§4.15) | + GPIO harness, ERM, LRA, nPM1300-EK |
...
**One ordering constraint runs backwards through this table:** M6 needs M4's connection-interval headroom number and M5's best-available LRA current-draw estimate (plus the ERM's, already measured at S15 and held for the fallback) as **design inputs** — it does not wait for R3, R4 or R6 to be *confirmed*, because none of the three can be, before the enclosure and strap that determine them exist. §4.15, §7.
...
The DK's GPIO carries what its onboard peripherals could not: the three remaining buttons and four RGB indicators, **the LRA on the same driver IC the ERM fallback candidate already uses**, and the nPM1300-EK. **This is where R3, R4 and R6 get their design-target estimate for M6** (§4.15) — the most representative rig that can exist before the enclosure does, understood as a number M6 designs against, not a closed measurement. Order follows §4.15's own logic: build what's fully specified first (buttons, indicators), then the items still needing a real-world component decision (haptic driver and motor selection, PMIC bring-up), so the open decisions don't block the closed ones.
...
- [x] **S15 [AGENT] — Haptic driver + ERM.** ✅ 2026-08-18 — DRV2605L selected and brought up on `i2c1` (P1.04/P1.05, §4.17); `remote/src/haptic.c` gained a real DRV2605L back end (ROM-library playback) behind the ERM path, `RATED_VOLTAGE`/`OD_CLAMP` derived from the Vybronics VZ7AL2B1690002 ERM datasheet by closed-form equation, not left as placeholders (§4.18). Factory calibration/OTP tooling relayed through the dongle (§4.17). **This closes S15 as originally scoped (one motor).** §4.19's later revision reassigns this work to the **fallback path** — the ERM selection, calibration and DRV2605L bring-up are all still correct and still needed, just not what the first prototype ships with by default. S21, below, is where the baseline's actual motor is chosen.
- [ ] **S21 [AGENT] — Select and bring up the LRA — the first prototype's haptic motor.** Per §4.19 (`SCOPE.md` v1.3 / `SYSTEM_FUNC_SPEC.md` v2.3): the LRA carries the *whole* haptic range for the first prototype — acknowledgement and the secondary-clock heartbeat at normal amplitude, plus the low-frequency expiry/warning notification class at overdrive. It is the product's motor under this design; the ERM built at S15 stays on the bench as the retained fallback candidate (§15.3), not the baseline. **A bench evaluation harness for exactly this choice already exists in the working tree, uncommitted** (`remote_haptic_eval/`) — standalone DRV2605L shell firmware carrying three actuator profiles (the ERM, plus two LRA candidates: Vybronics VG0832012 and JYLRA1030Z, both 1.8 Vrms coin LRAs) with per-actuator register derivation (`haptic act`/`cal`/`play`/`measure`) so the two LRAs can be A/B'd by ear and by measured resonance lock before either is committed to the product build. **No LRA candidate is chosen yet** — this step is that choice, plus wiring the winner into `remote/src/haptic.c` as the primary haptic path (replacing the ERM proxy `haptic.c` currently drives), while keeping the existing ERM path in place, unused in normal operation but ready to be swapped back in if §15.3's evaluation forces the fallback. Extend `DN_CONFIG` scaling to whatever amplitude control the real driver exposes on the LRA channel. Preserve the `BEAT`:`TAP` amplitude separation on the LRA (FS §11.1) — both events now share the sole motor, not a notionally-split duty.
- [ ] **S16 [BENCH] — Wire the LRA, strap the assembly to a wrist.** Confirm the LRA's acknowledgement, heartbeat and overdrive notification waveforms each render with the right character end to end, across the whole haptic range on one motor (§4.19, FS §15.3) — this is the rung that actually tests the single-LRA hypothesis. Take R3/R4's first real reading — **a design-target estimate for M6, not closure** (§4.15); M7 is where it closes. Keep S15's ERM wired and swappable on the same bench, so the fallback comparison (§15.3) can be run without a rebuild.
...
- [ ] **S19 [BENCH] — The M5 demonstration.** All of it together on one DK — seven buttons, four RGB indicators, the LRA carrying the whole haptic range, real battery — modelling the complete remote FS §3.1/§3.2/§3.3 describes. **This is what M6's component selection and layout get designed against.** *Closes M5.*
...
**M6 — the custom remote PCB.** No firmware runs. Inputs are M4's connection-interval headroom and M5's LRA current-draw estimate (the ERM's, already measured at S15, held in reserve for the fallback) — best-available numbers, designed against, not measurements the board waits to be confirmed. Nothing about the hardware design is recorded anywhere in this repository yet. Open items at least: module selection (an MDBT50Q variant keeps the RF characterisation), the LRA and driver chosen at M5/S21 — the first prototype's sole haptic motor, with the ERM built at S15 held as the fallback candidate rather than fitted by default (§4.19, `SCOPE.md`/`SYSTEM_FUNC_SPEC.md`) — nPM1300 as the production part, battery chemistry and capacity sized from R6's estimate with headroom (§4.8), the FS §3.1 button mechanics and oversized `TOGGLE_CLOCK` datum, four RGB indicators adjacent to their buttons, USB-C charging with a hardware-only bicolour `LED_CHG` charging-state indicator wired off the charge-management IC rather than the MCU (FS §3.4, §10.4), APPROTECT as a manufacturing step, and **a DFU strategy for the remotes, which exists in no document** (§8). **If M7 finds an estimate wrong, that is a second PCB spin** — an accepted cost of this ordering, not evidence the ordering was wrong (§4.15).
...
- You have one nPM1300-EK. A second haptic-capable remote means a second PMIC evaluation board and a second LRA (plus a second ERM, if the fallback bench needs duplicating too) before it means a second DK, so the DK is not even the binding purchase.
...
So: **no second DK for M4, and probably not for M5 either.** Revisit only if the M6 spin slips far enough that two-wrist evaluation on breadboards becomes the critical path — and if that happens, price a second nPM1300-EK and motor set in the same breath, because a DK alone would not unblock it.
...
**Bench-proven on the DK before any of this existed:** `remote/src/drv2605_cal_test.c` (now a thin wrapper over the real driver, `remote/src/drv2605.c`) validated the I2C wiring (`i2c1` on P1.04/P1.05 — every P0 pin was already claimed, §2 of `remote/BUILD_SPEC.md`) and the register-level auto-cal sequence against real hardware first. `RATED_VOLTAGE`/`OD_CLAMP` were left as deliberately unset placeholders through all of the above — **superseded 2026-08-18, §4.18**, which derives and sets both from the real Vybronics motor datasheet.

### 4.18 `RATED_VOLTAGE`/`OD_CLAMP` derived from the Vybronics motor datasheet, not left as placeholders; `haptic.c` gets a real DRV2605L back end

§4.17 assumed these two registers would need bench characterization against real hardware before they could be anything but a guess. That assumption was wrong, or at least stronger than the facts warranted — the datasheet gives closed-form equations (`datasheets/drv2605l.pdf` §8.5.2), and the Vybronics VZ7AL2B1690002's own spec sheet (`datasheets/Vybronics-VZ7AL2B1690002-datasheet.pdf`) gives everything both equations need: rated voltage 3.0V, operating range 2.2–3.6V.

- **`RATED_VOLTAGE[7:0] = 142` (`0x8E`).** Equation 4, closed-loop ERM: `V = 0.02118 × RATED_VOLTAGE[7:0]`. No dependencies beyond the motor's own rated voltage — solved exactly.
- **`OD_CLAMP[7:0] = 167` (`0xA7`).** Targets the motor's 3.6V operating ceiling — safe to drive continuously per its own datasheet, so a brief overdrive kick to that level is inside spec, not a stress test. The *exact* closed-loop formula (Equation 8) scales by a `DRIVE_TIME`/`IDISS_TIME`/`BLANKING_TIME`-dependent ratio that needs actuator-response tuning no motor datasheet provides. Used the simpler open-loop-style relationship (Equation 6) instead, deliberately, as a **provably conservative substitute**: Equation 8's timing ratio is always ≤ 1 (its numerator subtracts 300 µs its denominator doesn't add back), so at this register value the real closed-loop clamp voltage is at or below 3.6V for any legal timing setting, never above it. Not exact; safe by construction regardless.
- **Both are independent of VIN** — the equations relate register code to output voltage directly; VIN only caps what's reachable. On the DK bench build (VIN on the 3.3V rail, §-adjacent wiring discussion), `RATED_VOLTAGE`'s 3.0V target is reachable but `OD_CLAMP`'s 3.6V is supply-limited — expect a weaker-than-final overdrive kick on the DK specifically, not a bug. Full headroom arrives with the PCB's battery-fed VIN.
- Both constants now live in `remote/src/drv2605.c` (`RATED_VOLTAGE_TARGET`/`OD_CLAMP_TARGET`) and are written by `drv2605_calibrate()` before every calibration run, replacing the earlier "left at whatever the chip currently holds" behaviour.

**`haptic.c` gained a second back end, `CONFIG_REMOTE_HAPTIC_PROXY_LED=n`:** real DRV2605L ROM-library playback (`drv2605_play_sequence()`, new in `drv2605.c`) instead of the PWM/LED1 proxy, selected by a Kconfig `remote/BUILD_SPEC.md` §10 had documented since before it was implemented. Waveform → library effect mapping (`datasheets/drv2605l.pdf` §12.1.2) is a bench-verify-first starting point: `TAP`→1 ("Strong Click – 100%"), `BEAT`→3 ("Strong Click – 30%" — same click family as `TAP`, just weaker, satisfying `BUILD_SPEC.md` §7.3's "distinctly weaker, not different in principle" requirement by construction), `WARN`→7, `BUZZ`→47, `LONG`→14, `DOUBLE`→10, `TRIPLE`→effect 1 chained three times via `WAV_FRM_SEQ` (no dedicated triple-click entry found in the library list). The PWM backend's hand-timed pulse/gap state machine is gone on this path — the DRV2605L renders each effect's timing itself once triggered; a lightweight timer replaces it purely for BEAT-vs-TAP priority bookkeeping (RP §8.1), not for driving the motor.

**Known gap, not silently dropped: `DN_CONFIG`'s `haptic_scale` isn't applied on the DRV2605L path yet.** The obvious mechanism — scale `OD_CLAMP` at runtime — is wrong, not just unimplemented: `OD_CLAMP` is a calibration *input*, and the datasheet is explicit that changing it without recalibrating leaves `A_CAL_BEMF` stale. Real amplitude control needs per-tier effect selection (the same "different index, not a scaled shared parameter" approach already used for `BEAT` vs `TAP`), which needs the felt bench pass this whole mapping is waiting on regardless.

### 4.19 The first prototype's haptic architecture is a single LRA, not the ERM — two motors is the retained fallback, not the design; the heartbeat gains an operator toggle — `SCOPE.md` v1.3 / `SYSTEM_FUNC_SPEC.md` v2.3, 2026-09-02

`SCOPE.md` and `SYSTEM_FUNC_SPEC.md` are revised, superseding the single-ERM design adopted 2026-08-09 (v1.1/v2.1) that S15/§4.15/§4.17/§4.18 and the original R3/R4/R6 (§7) were all written against: **the first prototype PCB carries one motor, an LRA** — not the ERM S15 built. The LRA carries the entire haptic range: per-press acknowledgement and the secondary-clock heartbeat at normal amplitude, and the low-frequency, high-stakes notification class (activity/period/match clock expiry, the five-second warning, ADCC phase boundary, link-lost, low battery) driven at overdrive — the towel-tapper replacement (*Project Scope* §1.2). FS §3.3 states the rationale directly: this is the cheaper hypothesis to test first, since one well-chosen LRA might cover both amplitude regimes in the full wrist assembly (enclosure and strap included), and a second PCB iteration is owed only if it can't. New alongside it, unaffected by which motor architecture ships: the secondary-clock heartbeat can be enabled or disabled live from the scoreboard (FS §8.7), default enabled, independent of `LED_F1` ownership rendering.

**A dual-motor design — ERM for notification, LRA for acknowledgement and heartbeat — was drafted in the same revision and retired before it landed anywhere outside this working tree.** It reasoned that an ERM sized for expiry amplitude is the wrong motor for acknowledgement's repetition count and the 120 ms budget, while an LRA good for acknowledgement is too small to be felt as an unmistakable expiry buzz — both true, but an argument for testing whether one well-chosen LRA can do both jobs before committing to two motors, not for building two motors directly. FS §3.3 keeps that reasoning on record specifically to justify the fallback it retains. **No prior committed state of this document ever described the dual-motor architecture as settled** — the last version actually adopted (`HISTORY.md`, 2026-08-09) was the single-ERM design this entry supersedes directly; a same-session draft describing "two motors, settled" existed only in this working tree and is corrected here rather than carried forward.

**Two motors is not gone — it is the retained fallback**, taken as a second PCB iteration only if M5/S21's bench-and-wrist evaluation (FS §15.3, the decision point) shows the single LRA can't reliably deliver the notification class, or can't hold the ten-hour budget (R6) carrying everything. A single-ERM design (the pre-2026-08-09 shape) is the fallback of the fallback, if even two motors turns out not to be justified.

**Consequence for the queue (§2.3):** S15 (DRV2605L bring-up, ERM selection and calibration, §4.17/§4.18, closed 2026-08-18) is retained in full, but its output is now the **fallback candidate's** motor, not the baseline's — still correct, still needed, just not what the first prototype ships with unless §15.3 forces it. **S21** is the step that now matters for the baseline: selecting an LRA candidate and bringing it up as the product's sole haptic path for the first prototype. S16 (wire the motor, strap to a wrist) is reframed around the LRA as the primary rung — the one that actually tests the single-LRA hypothesis — while keeping S15's ERM wired and swappable on the same bench so the fallback comparison doesn't need a rebuild.

**Consequence for R3/R4/R6 (§7):** R3 is reframed a second time — from "one ERM covers everything" (pre-2026-08-09), through the retired "LRA covers acknowledgement+heartbeat, ERM alone covers notification" split, to its current form: "does the LRA cover the *whole* range, notification included." R3 is now also FS §15.3's decision point, where the fallback is triggered or not. R4 (amplitude separation, `BEAT` vs `TAP`) is unchanged in substance — both events are on the LRA under every architecture this document has considered. R6 (battery) now measures the single LRA carrying everything as the primary case; the ERM's separate per-event draw becomes a real measurement only if the fallback is triggered.

**Not yet decided: which LRA.** A bench evaluation harness for exactly this choice exists in the tree already, uncommitted (`remote_haptic_eval/`, described at S21) — it carries the ERM (the fallback candidate) alongside two named LRA candidates (Vybronics VG0832012, JYLRA1030Z), so the LRA-candidate choice and the architecture question (§15.3) can both be explored on the same bench. No candidate has been chosen; S21 is where that happens.
...
| **W4** | S20, PCB fab wait | **Round trip and the deadline rule** — not required to close M2 (§4.16) | The app's own EVT→ACK turnaround measured as a distribution (not the full radio-inclusive round trip — neither leg has a wire timestamp, `HISTORY.md` §9.2). A8 a late `ACK` is not sent at all, `taps_dropped_late` non-zero when provoked and zero when not. A10 a `BEAT` never truncates a `TAP` — re-run against M5's real LRA (§4.19), not the LED proxy. **A9 excluded — mechanism 2 is not implemented, §8** |
...
| B8 | **Heartbeat contention with acknowledgement.** The beat and the tap share the LRA (§4.19), and a four-press burst takes about a second, so they *will* collide (FS §11.1). | Confirm burst suppression in the app, and confirm amplitude separation on real hardware with a real strap. |
...
### R3 — The LRA has to cover the whole haptic range, notification included — **design target, closed at M7, §4.15, reframed §4.19; also FS §15.3's decision point**

Previously framed as one ERM covering the whole haptic range; briefly drafted mid-revision as two narrower questions (an LRA half and an ERM half) before that split was itself retired (§4.19). **Reframed 2026-09-02 (§4.19)** to ask the original question of the LRA instead: FS §3.3/§15.3 now assume the LRA plus driver IC delivers, in one motor, all three of a countable reduced-amplitude heartbeat, an acknowledgement inside 120 ms, and an unmistakable overdrive expiry/warning buzz. Motor spin-up is allocated 20 ms of the acknowledgement budget and is the hard floor. None of it can be settled from datasheet figures, **and it cannot be fully settled from a DK-wired breadboard motor either** — the mechanical coupling that actually delivers the sensation (housing, strap material and tension) doesn't exist until the enclosure does. M5/S21 measures the best rig that can exist before then and treats the result as a design target, not a closed question; M7 is where it closes for real, against the actual enclosure and strap. **This is also the decision point for the whole haptic architecture** (FS §15.3): if the single LRA doesn't reliably deliver the notification class, or R6's ten-hour budget doesn't hold with one motor carrying everything, the design reverts to the retained two-motor fallback (§4.19) or, failing that, a single ERM — a second PCB iteration, decided on the evaluation rather than before it. The mechanical-interference sub-question a two-motor revert would introduce (FS §15.7) only arises if that fallback is actually taken.

### R4 — Amplitude separation has to be perceptible — **design target, closed at M7, §4.15**

Whether the difference between `BEAT` and `TAP` is reliably distinguishable on the wrist, in motion, through a strap, by a referee not attending to it. Both are delivered on the LRA (§4.19), as they were on the single ERM before it, and stay on one shared motor under every haptic architecture this document has considered, fallback included — R4 doesn't depend on which one ships. This is the assumption repeated-press scoring rests on (FS §11.1, §15.5), and it fails quietly: a referee who miscounts a near fall has no way to know. Same limitation as R3: a bench rig can confirm the waveform table's *character* is right, not that it survives a real strap on a moving wrist. M5's confirmation is provisional; M7's is the one that counts.
...
Measure average current attributable to the radio at the connection cadence the acknowledgement budget requires, and to the single LRA delivering everything over a representative match — per-press acknowledgement, a reduced-amplitude 1 Hz beat, and the overdrive expiry/warning/notification events (§4.19, FS §15.2). Confirm the ten-hour target holds with one motor carrying all of it; if it does not, that's one of the two inputs (with R3) to the two-motor-fallback decision at §15.3, and only then does the ERM's separate per-event draw become a real measurement to take, rather than the S15 figure already on record. **The heartbeat toggle (FS §8.7) is the operator's power lever regardless of which architecture ships** — a live, visible, deliberate choice, not an automatic mode. **A DK-tethered current measurement is a component-level estimate, not a system one** — real draw depends on the production PCB's antenna efficiency and regulator losses, neither of which exist until M6 ships a board. M5 gives M6 a sizing target with headroom (§4.8); M7's ten-hour session is the actual measurement.
...
| **No hardware design work of any kind** | Module, the LRA and driver (ERM retained as the bench fallback candidate, §4.19), PMIC production part, battery sizing, button mechanics, enclosure, APPROTECT. M6, and its inputs are M4 and M5 measurements — §2 |
...
| **Haptics and indicators unvalidated on the surface that carries them** | No real LRA (or the ERM fallback), RGB or strap until M5, and even M5's rig is a design-target estimate, not closure — R3/R4/R6 don't close until M7, against the real enclosure (§4.15) |
| **LRA not yet selected** | Two candidates identified (Vybronics VG0832012, JYLRA1030Z) and a bench evaluation harness exists (`remote_haptic_eval/`, uncommitted), but no A/B comparison has run and no candidate is chosen. **S21** (§4.19) |
...
- [ ] The DK models a complete remote — seven buttons, four RGB indicators, the real LRA carrying the whole haptic range, real battery — **S19**. Closes M5

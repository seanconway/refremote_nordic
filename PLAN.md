...
| **M2** | **The firmware programme** — dongle wire v3.0, the radio, and a DK remote, to an end-to-end demonstration | ▶ in progress — §3.1. Stages 0–1 code-complete and **flashed**; V0–V4 green (V4 minus one UI-unreachable row); V5–V6 need physical acts |
...
| Validation | **V0–V4 all green**, 2026-08-11 — V0/V1 from a scripted terminal (§2.7), V2–V4 from the live application holding the port (§5, §9.2). The v2.0 passes of V1–V3 are **void** and have been superseded rather than carried. **V4's one open row — burst suppression under `ADD_POINT`s — cannot be driven from the operator UI at all**, since an on-screen click produces no wire traffic to acknowledge; it needs dongle-originated `EVT`s and is otherwise covered by a unit test. **V5 and V6 need physical acts** — unplug, sleep, kill-the-browser — that are the operator's hands, not a script's; V6.1/V6.3/V6.4 are the exception, reachable from the UI alone. §5 carries the rungs as per-stage exit criteria. |

**The two ends have been connected, in both directions.** The dongle runs 0.2.0 and speaks v3.0, the application has held the port multiple times independently, and V2–V4 are closed. **V5 — supervision and disconnection — is the next rung**, and it needs the operator's hands: unplug, sleep, kill-the-browser.

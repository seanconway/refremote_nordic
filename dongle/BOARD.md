...
`storage_partition` is where the provisioning record of `RADIO_PROTOCOL.md` §10.1 lives (stage 2). It sits **below** the nRF5 bootloader and **outside** `slot0_partition`, so it survives an ordinary application reflash — which is the property that makes a provisioned unit stay provisioned across firmware updates.

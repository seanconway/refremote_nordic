...
| **3** | `rframe` codec, then BLE at 7.5 ms | ◐ **Code-complete, W0 closed, 2026-08-12.** `common/rframe.c/.h` and `src/radio_ble.c` written and building clean under `-DCONFIG_DONGLE_RADIO=y`; `dongle/tests/rframe` — 20 checks, 67 assertions, `make check` green, 0 failures — after the host `as.exe` block (transient, not a code defect) cleared (PLAN.md §9.2/§9.3). **W1** (A12, A13, A14, A19 on the connection) remains to be run on hardware — a pass on the positive case alone is not a pass |

...
/* Reported in HELLO's <set> field only when PROV_RECORD fails validation or
 * carries the wrong role — BUILD_SPEC.md §9. Deliberately not a plausible
 * serial: the app displays this for the referee's pre-match check against
 * the label on the hardware (PROTOCOL.md §4.1, FS §12.3), and a placeholder
 * that looked real would pass that check silently. A live ERR NO_PROVISIONING
 * already fired at boot for the same condition (engine_start()), so this is
 * what a human sees on the screen, not the only signal of the fault. */
...
/* PROV_RECORD is a compile-time constant (provisioning_data.h) — this is set
 * once at boot by engine_start() validating it, not by reading anything. */
...
static void send_hello(void)
{
	...
	const char *set_serial = provisioned ? PROV_RECORD.set_serial : UNPROVISIONED_SET_SERIAL;
...
void engine_start(void)
{
	...
	/*
	 * Checked before anything else touches the wire: HELLO's <set> field
	 * below, and — from Stage 3 — radio_init()'s association parameters,
	 * both come from PROV_RECORD. BUILD_SPEC.md §9: on any failure the
	 * unit still runs the USB side of the protocol (there is no advertise
	 * or initiate to refuse yet with CONFIG_DONGLE_RADIO=n) but the fault
	 * is reported once, so a bench technician sees it rather than a
	 * dongle that looks fine and silently reports the wrong set. This is
	 * the boot-time sentinel discussed alongside the build-time refusal
	 * in CMakeLists.txt — belt and braces: the build can't produce a hex
	 * with no provisioning_data.h at all, but this still catches a header
	 * that exists and carries garbage (a generator bug, a hand edit).
	 */
	{
		enum provisioning_status st = provisioning_validate(&PROV_RECORD);

		provisioned = (st == PROVISIONING_OK) &&
			      (PROV_RECORD.role == PROVISIONING_ROLE_DONGLE);
		if (!provisioned) {
			char text[48];

			(void)snprintf(text, sizeof(text), "unprovisioned: %s",
				       st == PROVISIONING_OK ? "wrong role"
	...
	if (radio_init(&radio_callbacks, &engine_q, provisioned ? &PROV_RECORD : NULL) != 0) {

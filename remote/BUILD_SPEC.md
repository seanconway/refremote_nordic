...
| **4** | ◐ **W2 and W3 closed on hardware 2026-08-13** — `src/link.c`, `buttons.c`, `haptic.c`, `indicators.c`, `main.c`, all confirmed against a live scoreboard. **W2** uplink — all three gestures on all four DK buttons, `HOLD` firing on threshold-cross not release, non-repetition proven, `HOLD_REP` repeating and stopping cleanly on release. Exact cadence and A6 (wrap) not forced this session — the arithmetic is already host-suite-proven and neither blocks anything (`PLAN.md` §3). **W3** downlink — indicators assert and clear correctly, and **A20 confirmed by reading the wire log**, not by watching an LED that was never going to light; `HAP RED` and `CFG` observed on real hardware for the first time. **W4** round trip — A8, A9, A10, and `taps_dropped_late` non-zero when provoked and zero when not. **W5** link state — **A15**, A16, A17, A18, A7 — remain, `PLAN.md` queue steps S6–S7 |

...
| Stage | Green when |
|---|---|
| **3** | ◐ **W0 green 2026-08-12; W1's positive case confirmed on hardware 2026-08-13** — `common/rframe.c/.h` shared with the dongle build, host suite green (`dongle/tests/rframe`, 20 checks, 67 assertions, 0 failures — `HISTORY.md` §9.2/§9.3). **W0** — codec cases A1–A7, A11, A20 green on the host, with no board involved. **W1** — a DK (RED) connects, encrypted from the provisioned key, `RR_IDENTITY` read and validated, CCCD subscribed, `LED_LINK` (DK LED 4) solid, confirming the same from the remote's own side. The dongle's `set_serial` comparison in `handle_identity_read()` is now implemented (`PLAN.md` queue step S1, 2026-08-13). **The negatives A12, A13, A14, A19 are prepared and confirmed to compile but still to be run on hardware — a pass on the positive case alone is not a pass** — `PLAN.md` queue step S3 |
